...
## Cross-platform smoke (Windows + macOS)

The Linux smoke step in `cloudbuild.yaml:smoke-test` validates the cross-compiled Linux artifacts on the same Linux host that builds them — that is the only platform Cloud Build's private worker pools support. Cross-platform smoke for Windows and macOS requires SSH-tunneled remote runners following the same pattern as `cloudbuild-mac.yaml`. Both pipelines are committed to the repo as inert-until-host stubs.

`cloudbuild-mac.yaml` was already wired for SSH-tunneled signed DMG builds. It now also includes a `smoke-test-mac` step that runs Puppeteer-CDP against the freshly-built `RelayGate.app` on the same Mac host that produced it, gating `publish-mac-dmg` on the smoke result. The step is inert until `_MAC_RUNNER_HOST` is set via the trigger's `--substitutions`, exactly like the build step. Provisioning paths are documented at the top of the YAML and in `docs/MAC_BUILD.md`.

`cloudbuild-win.yaml` is new in this iteration. It is structurally identical to the Mac pipeline: a single SSH-driven step that pulls the `RelayGate-Setup-0.1.0.exe` produced by the main Linux pipeline (from the env-prefixed GCS path), SCPs it to the Windows runner, runs the nsis silent installer, then invokes the Puppeteer-CDP smoke against the installed binary at `%LOCALAPPDATA%\Programs\RelayGate\RelayGate.exe`. The step is inert until `_WIN_RUNNER_HOST` is set. Provisioning options for the Windows host are listed at the top of the YAML; the cheapest is a Windows NUC at the office, the most expensive-but-managed is a GCE Windows Server 2022 instance (which would require expanding the `relaygate-desktop-ci@` SA with `roles/compute.instanceAdmin.v1` — that contradicts the recent SA scope-down audit, so it is gated on explicit user authorization, see `audit/scope-findings/2026-05-05-windows-smoke-blocked.md`).

The choice to ship both pipelines as inert stubs rather than wire them into the main `cloudbuild.yaml` `publish` step's `waitFor` chain is deliberate. Wiring an always-no-op step into `publish.waitFor` would silently mark publish as "smoke-tested on Windows" when in fact the smoke is skipping every run — a perma-pass that defeats the point of the gate. The honest posture is "Windows + macOS smoke are designed and ready to run; they will start running the moment a runner host is wired up." The first commit that flips a runner from empty to set is the commit that actually starts gating publish on cross-platform smoke; until then, the unsigned mac.zip and unsigned Windows nsis exe are produced and shipped without on-target smoke validation, exactly as they have been from day one.
