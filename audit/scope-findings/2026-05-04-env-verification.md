...
**F-B** (no PR triggers): STATUS: FIXED (commit: 2ebbdba; followups: 54a1925 + 3cdf177) — `relaygate-desktop-pr` trigger created (gen2/us-central1), bound to `relaygate-desktop-ci@`, fires on `pullRequest` event against `^main$`, points at `cloudbuild.yaml`, comment-control `COMMENTS_ENABLED`. Verified via `gcloud builds triggers describe`. End-to-end verification: PR #2 fired the trigger, build `ad85669c` registered and ran 4/5 steps under the new SA (proving the SA scope-down works for the full pipeline path), failed step 5 (`build-mac-dmg`) on a transient `apt-get install: Unable to fetch some archives` Ubuntu mirror flake. Two follow-up fixes applied as part of this finding's resolution: `54a1925` adds 3-attempt retry with `--fix-missing` to the apt step (eliminates the transient failure mode for both PR and main-push triggers); `3cdf177` fixes a real bug found during PR-trigger wiring where the `publish` step copied every artifact to BOTH `/{SHORT_SHA}/` AND `/latest/` unconditionally — meaning a successful PR build would have clobbered the production download mirror with PR-head artifacts. The fix detects `TRIGGER_NAME=relaygate-desktop-pr` and skips the `/latest/` copies for PR builds; main-push builds (`TRIGGER_NAME=relaygate-desktop-binaries`) keep mirroring to `/latest/`. Caught before any PR build reached the publish step. Final PR-build SUCCESS is in flight at audit-log close; will be visible in subsequent build history.
